...
public:
	...
	{
		carla::sensor::s11n::SensorHeaderSerializer::Header* HeaderStr = reinterpret_cast<
			carla::sensor::s11n::SensorHeaderSerializer::Header*>(Header.data());
		if (HeaderStr)
		{
			if (HeaderStr->frame != FrameNumber)
			{
				carla::log_debug("Re-framing sensor type ", HeaderStr->sensor_type, " from ", HeaderStr->frame, " to ",
				                 FrameNumber);
