...
#include "Traffic/TrafficLightGroup.h"
#include "EngineUtils.h"

#include "OpenDrive/OpenDrive.h"
#include "Util/DebugShapeDrawer.h"
#include "Util/NavigationMesh.h"
...
#include "Vehicle/MovementComponents/CarSimManagerComponent.h"
#include "Vehicle/MovementComponents/ChronoMovementComponent.h"
#include "Lights/CarlaLightSubsystem.h"
...
void FCarlaServer::FPimpl::BindActions()
{
	...
	BIND_SYNC(load_new_episode) << [this](const std::string& map_name, const bool reset_settings,
	                                      cr::MapLayer MapLayers) -> R<void>
	{
	...
	BIND_SYNC(copy_opendrive_to_file) << [this](const std::string& opendrive,
	                                            cr::OpendriveGenerationParameters Params) -> R<void>
	{
		REQUIRE_CARLA_EPISODE();
	...
	// ~~ GBuffer tokens ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	BIND_SYNC(get_gbuffer_token) << [this](const cr::ActorId ActorId,
	                                       uint32_t GBufferId) -> R<std::vector<unsigned char>>
	{
		REQUIRE_CARLA_EPISODE();
		FCarlaActor* CarlaActor = Episode->FindCarlaActor(ActorId);
		if (!CarlaActor)
...
#define MAKE_RESULT(operation) return parse_result(c.actor, operation);
	...
	auto command_visitor = carla::Functional::MakeRecursiveOverload(
		...
		[=](auto, const C::ApplyVehicleAckermannControl& c)
		{
		...
		[=](auto, const C::ApplyVehiclePhysicsControl& c)
		{
		...
		[=](auto, const C::ApplyTargetAngularVelocity& c)
		{
		...
		[=](auto, const C::SetTrafficLightState& c)
		{
