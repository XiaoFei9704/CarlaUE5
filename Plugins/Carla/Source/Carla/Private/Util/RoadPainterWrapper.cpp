...
ARoadPainterWrapper::ARoadPainterWrapper()
{
	...
	for (int32 i = 0; i < MeshActors.Num(); ++i)
	{
		...
		if (StaticMeshActor)
		{
			if (StaticMeshActor->GetName().Contains("Curb", ESearchCase::Type::IgnoreCase) || StaticMeshActor->GetName()
				.Contains("Gutter", ESearchCase::Type::IgnoreCase))
			{
			...
			for (auto& DecalJsonValue : DecalJsonArray)
			{
				...
				if (JsonMapName.Equals(CurrentMapName) == true)
				{
					...
					for (const TPair<FString, FString>& Pair : DecalNamesMap)
					{
						...
						{
							DecalPropertiesConfig.DecalMaterials.Add(
					...
					DecalPropertiesConfig.DecalScale = ReadVectorFromJsonObject(
						DecalJsonObject->GetObjectField(TEXT("decal_scale")));
					DecalPropertiesConfig.FixedDecalOffset = ReadVectorFromJsonObject(
						DecalJsonObject->GetObjectField(TEXT("fixed_decal_offset")));
					DecalPropertiesConfig.DecalMinScale = (float)DecalJsonObject->GetNumberField(
						TEXT("decal_min_scale"));
					DecalPropertiesConfig.DecalMaxScale = (float)DecalJsonObject->GetNumberField(
						TEXT("decal_max_scale"));
					DecalPropertiesConfig.DecalRandomYaw = (float)DecalJsonObject->GetNumberField(
						TEXT("decal_random_yaw"));
