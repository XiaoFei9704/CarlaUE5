...
void ACarlaWheeledVehicle::BeginPlay()
{
	...
	for (FName& ComponentName : ConstraintComponentNames)
	{
		...
		if (ConstraintComponent)
		{
			...
			{
				UE_LOG(LogCarla, Error, TEXT("Missing component for constraint: %s"),
...
void ACarlaWheeledVehicle::UpdateDetectionBox()
{
	...
	const FTransform NonScaledTransform(GlobalTransform.GetRotation(), GlobalTransform.GetLocation(),
	                                    {1.0f, 1.0f, 1.0f});
...
void ACarlaWheeledVehicle::ApplyRolloverBehavior()
{
	...
	{
	case 0: CheckRollover(roll, std::make_pair(130.0, 230.0));
		break;
	case 1: CheckRollover(roll, std::make_pair(140.0, 220.0));
		break;
	case 2: CheckRollover(roll, std::make_pair(150.0, 210.0));
		break;
	case 3: CheckRollover(roll, std::make_pair(160.0, 200.0));
		break;
	case 4:
		GetWorld()->GetTimerManager().SetTimer(TimerHandler, this, &ACarlaWheeledVehicle::SetRolloverFlag,
		                                       RolloverFlagTime);
