...
#include "Vehicle/VehicleMovementComponentNW.h"
...
static void GetVehicleDifferentialNWSetup(const TArray<FVehicleNWWheelDifferentialData>& Setup,
                                          PxVehicleDifferentialNWData& PxSetup)
{
...
void SetupDriveHelper(const UWheeledVehicleMovementComponentNW* VehicleData,
...
void UWheeledVehicleMovementComponentNW::UpdateSimulation(float DeltaTime)
{
	...
	FPhysicsCommand::ExecuteWrite(TargetInstance->ActorHandle, [&](const FPhysicsActorHandle& Actor)
	{
		...
		PxVehiclePadSmoothingData SmoothData = {
			{
				ThrottleInputRate.RiseRate, BrakeInputRate.RiseRate, HandbrakeInputRate.RiseRate,
				SteeringInputRate.RiseRate, SteeringInputRate.RiseRate
			},
			{
				ThrottleInputRate.FallRate, BrakeInputRate.FallRate, HandbrakeInputRate.FallRate,
				SteeringInputRate.FallRate, SteeringInputRate.FallRate
			}
		};

		PxVehicleDriveNW* PVehicleDriveNW = (PxVehicleDriveNW*)PVehicleDrive;
		PxVehicleDriveNWSmoothAnalogRawInputsAndSetAnalogInputs(SmoothData, SpeedSteerLookup, RawInputData, DeltaTime,
...
void UWheeledVehicleMovementComponentNW::UpdateDifferentialSetup(
...
void UWheeledVehicleMovementComponentNW::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading() && Ar.UE4Ver() < VER_UE4_VEHICLES_UNIT_CHANGE)
	{
		...
		//need to convert from rad/s to RPM
	}
	...
	if (Ar.IsLoading() && Ar.UE4Ver() < VER_UE4_VEHICLES_UNIT_CHANGE2)
	{
		...
		BackwardsConvertCm2ToM2NW(EngineSetup.DampingRateZeroThrottleClutchDisengaged,
		                          DefEngineData.mDampingRateZeroThrottleClutchDisengaged);
		BackwardsConvertCm2ToM2NW(EngineSetup.DampingRateZeroThrottleClutchEngaged,
		                          DefEngineData.mDampingRateZeroThrottleClutchEngaged);
...
const physx::PxVehicleWheelData UWheeledVehicleMovementComponentNW::GetWheelData(
	physx::PxVehicleWheels* InWheels, uint32 InWheel)
{
