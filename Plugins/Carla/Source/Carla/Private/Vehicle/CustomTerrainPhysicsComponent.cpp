				...
				while (ConditionToStopWaiting)
				{
					uint64_t CurrentTileID = GetTileId(X, Y);
					{
						TRACE_CPUPROFILER_EVENT_SCOPE(
							TEXT("FSparseHighDetailMap::WaitUntilProperTilesAreLoaded::TimeLocked"));
...
void FSparseHighDetailMap::SaveMap()
{
	...
	ParallelFor(Map.size(), [this](int32 Idx)
	{
		...
		std::string FileToSavePath = std::string(
			TCHAR_TO_UTF8(*( this->SavePath + it.second.TilePosition.ToString() + ".tile")));
		std::ofstream OutputStream(FileToSavePath.c_str());
...
void UCustomTerrainPhysicsComponent::UpdateLargeTextureData()
{
	...
	uint32_t NumberOfParticlesIn1AxisInLargeHeightmap = LargeTextureToUpdate->GetSizeX() / (CacheRadius.X * 2) /
		TileSize;

	std::vector<float> ParticlesPositions;
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(
			TEXT("UCustomTerrainPhysicsComponent::UpdateLargeTextureData::GettingHeightMaps"));
	...
	while (OffsetY < LargeTextureToUpdate->GetSizeY())
	{
		...
		{
			float LocalYTransformed = std::floor(
				y / NumberOfParticlesIn1AxisInLocalHeightmap * NumberOfParticlesIn1AxisInLargeHeightmap);
			for (uint32_t x = 0; x < NumberOfParticlesIn1AxisInLocalHeightmap; x++)
			{
				...
				float LocalXTransformed = std::floor(
					x / NumberOfParticlesIn1AxisInLocalHeightmap * NumberOfParticlesIn1AxisInLargeHeightmap);
...
void UCustomTerrainPhysicsComponent::BuildLandscapeHeightMapDataAasset(ALandscapeProxy* Landscape,
                                                                       int Resolution, FVector MapSize,
                                                                       FString AssetPath, FString AssetName)
{
	...
	UHeightMapDataAsset* HeightMapAsset = NewObject<UHeightMapDataAsset>(
	...
	bool bSaved = UPackage::SavePackage(Package, HeightMapAsset, EObjectFlags::RF_Public | EObjectFlags::RF_Standalone,
                                                   ...
                                                   ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	for (AActor* VehicleActor : VehiclesActors)
	{
		...
		if (LargeMapManager)
		{
			...
			if (CurrentLargeMapTileId != CurrentTileId)
			{
				...
				if (LargeMapTile)
				{
					...
					UObject* DataAssetObject = StaticLoadObject(UHeightMapDataAsset::StaticClass(), nullptr,
					                                            *(AssetPath));
					if (DataAssetObject)
					{
						...
						{
							FVector TilePosition = HeightMapOffset + LargeMapManager->
								GetTileLocation(CurrentLargeMapTileId) - 0.5f * FVector(
							...
							UE_LOG(LogCarla, Log, TEXT("Updating height map to location %s in tile location %s"),
							       ...
							       *LargeMapManager->GetTileLocation(CurrentLargeMapTileId).ToString());
		...
		if (bDrawLoadedTiles)
		{
			...
			DrawTiles(GetWorld(), SparseMap.GetTileIdInCache(), GlobalLocation.Z + 300,
			          FLinearColor(1.0, 0.0, 0.0, 0.0));
		}
	...
	if (bDrawHeightMap)
	{
		...
		(World
			 ? (bDepthIsForeground
				    ? World->ForegroundLineBatcher
				    : ((bPersistentLines || (LifeTime > 0.f)) ? World->PersistentLineBatcher : World->LineBatcher))
			 : nullptr);
		if (!LineBatcher)
		{
...
void UCustomTerrainPhysicsComponent::OnLevelAddedToWorld(ULevel* InLevel, UWorld* InWorld)
{
	if (bRemoveLandscapeColliders)
	{
		for (auto CurrentActor : InLevel->Actors)
		{
			...
			{
				CurrentLandscape->BodyInstance.ReplaceResponseToChannels(
					ECollisionResponse::ECR_Block, ECollisionResponse::ECR_Ignore);
				CurrentLandscape->BodyInstance.ReplaceResponseToChannels(
					ECollisionResponse::ECR_Overlap, ECollisionResponse::ECR_Ignore);
	...
	const std::vector<FParticle*>& Particles)
{
	...
	for (size_t i = 0; i < Particles.size(); i++)
	{
		...
		if (Index < CurrentTile.ParticlesZOrdered.size())
		{
			...
			{
				UE_LOG(LogCarla, Error, TEXT("Cannot Remove more from %d, currentsize %d Tile : %s"), Index,
				       CurrentTile.ParticlesZOrdered[Index].size(), *(TilePosition.ToString()));
			}
		}
		else
		{
			UE_LOG(LogCarla, Error,
			       TEXT("RemoveParticlesFromOrderedContainer Invalid Index %d ZOrderedSize %d Tile: %s"), Index,
			       CurrentTile.ParticlesZOrdered.size(), *(TilePosition.ToString()));
		}
	}
}
...
void UCustomTerrainPhysicsComponent::AddForceToSingleWheel(USkeletalMeshComponent* SkeletalMeshComponent,
                                                           FVector WheelPosition, FVector WheelNormalForce)
{
	...
	SkeletalMeshComponent->AddForceAtLocationLocal(WheelPosition,
	                                               SIToUEFrame(
		                                               WheelNormalForce) * (ForceFactor * NormalForceIntensity));
}
	...
	FVector TorqueWheel0, FVector TorqueWheel1, FVector TorqueWheel2, FVector TorqueWheel3)
{
	...
	if (bDisableVehicleGravity&& PrimitiveComponent
	->
	IsGravityEnabled()
	)
	{
	...
	if (DrawDebugInfo && bShowForces)
	{
		...
		LineBatcher->DrawLine(WheelPosition0 + FVector(0, 0, 50),
		                      WheelPosition0 + FVector(0, 0, 50) + ForceWheel0.GetSafeNormal() * (5 + ForceMulFactor *
			                      10 * ForceWheel0.Size()),
		                      FLinearColor(0.0, 1.0, 0.0), 0, 3.0, 0.3);
		LineBatcher->DrawLine(WheelPosition1 + FVector(0, 0, 50),
		                      WheelPosition1 + FVector(0, 0, 50) + ForceWheel1.GetSafeNormal() * (5 + ForceMulFactor *
			                      10 * ForceWheel1.Size()),
		                      FLinearColor(0.0, 1.0, 0.0), 0, 3.0, 0.3);
		LineBatcher->DrawLine(WheelPosition2 + FVector(0, 0, 50),
		                      WheelPosition2 + FVector(0, 0, 50) + ForceWheel2.GetSafeNormal() * (5 + ForceMulFactor *
			                      10 * ForceWheel2.Size()),
		                      FLinearColor(0.0, 1.0, 0.0), 0, 3.0, 0.3);
		LineBatcher->DrawLine(WheelPosition3 + FVector(0, 0, 50),
		                      WheelPosition3 + FVector(0, 0, 50) + ForceWheel3.GetSafeNormal() * (5 + ForceMulFactor *
			                      10 * ForceWheel3.Size()),
		                      FLinearColor(0.0, 1.0, 0.0), 0, 3.0, 0.3);
	}
}
	...
	FVector ForceWheel0, FVector ForceWheel1, FVector ForceWheel2, FVector ForceWheel3)
{
	...
	if (DrawDebugInfo && bShowForces)
	{
		...
		LineBatcher->DrawLine(PrimitiveComponent->GetComponentLocation() + FVector(0, 0, 200),
		                      PrimitiveComponent->GetComponentLocation() + FVector(0, 0, 200) + MeanAcceleration.
		                      GetSafeNormal() * (1 + MeanAcceleration.Size()),
		                      FLinearColor(0.0, 1.0, 0.0), 0, 3.0, 0.3);
	}
}
...
FTilesWorker::FTilesWorker(UCustomTerrainPhysicsComponent* TerrainComp, FVector NewPosition, float NewRadiusX,
                           float NewRadiusY)
{
