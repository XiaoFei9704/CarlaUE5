	...
	std::unordered_map<uint32_t, uint32_t>& MappedId)
{
	...
	else if (SpawnSensors || !ActorDesc.Id.StartsWith("sensor."))
	{
		...
		else if (MappedId.find(DesiredId) != MappedId.end() && Episode->GetActorRegistry().
		                                                                Contains(MappedId[DesiredId]))
		{
...
// reposition actors
bool FFrameData::ProcessReplayerPosition(CarlaRecorderPosition Pos1, CarlaRecorderPosition Pos2, double Per,
                                         double DeltaTime)
{
	check(Episode != nullptr);
...
// replay finish
bool FFrameData::ProcessReplayerFinish(bool bApplyAutopilot, bool bIgnoreHero,
