	...
	if (GetCurrentEpisode())
	{
		if (bIsPrimaryServer)
		{
			if (SecondaryServer->HasClientsConnected())
			{
				...
				SecondaryServer->GetCommander().SendFrameData(
					carla::Buffer(std::move((unsigned char*)Tmp.c_str()), (size_t)Tmp.size()));
