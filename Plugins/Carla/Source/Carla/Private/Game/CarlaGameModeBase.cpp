...
void ACarlaGameModeBase::BeginPlay()
{
	...
	if (true)
	{
		/// @todo If semantic segmentation enabled.
	...
	if (ReadyToRegisterObjects&& PendingLevelsToLoad
	==
	0
	)
	{
...
UTexture2D* ACarlaGameModeBase::CreateUETexture(const carla::rpc::TextureColor& Texture)
{
	...
	UTexture2D* UETexture = UTexture2D::CreateTransient(Texture.GetWidth(), Texture.GetHeight(),
...
void ACarlaGameModeBase::GenerateSpawnPoints()
{
	...
	std::vector<std::pair<carla::road::element::Waypoint, carla::road::element::Waypoint>> Topology = Map->
		GenerateTopology();
	UWorld* World = GetWorld();
...
ATrafficLightManager* ACarlaGameModeBase::GetTrafficLightManager()
{
	...
	{
		UWorld* World = GetWorld();
		AActor* TrafficLightManagerActor =
			UGameplayStatics::GetActorOfClass(World, ATrafficLightManager::StaticClass());
...
void ACarlaGameModeBase::OnUnloadStreamLevel()
{
	...
	if (ReadyToRegisterObjects&& PendingLevelsToUnLoad
	==
	0
	)
	{
