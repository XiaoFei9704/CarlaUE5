...
#include "Vegetation/SpringBasedVegetationComponent.h"
#include "Walker/WalkerAnim.h"
...
void USpringBasedVegetationComponent::GenerateSkeletonHierarchy()
{
	...
	for (int i = 0; i < Skeleton.Joints.Num(); ++i)
	{
		...
		FTransform JointTransform = SkeletalMesh->GetSocketTransform(FName(*Joint.JointName),
		                                                             ERelativeTransformSpace::RTS_ParentBoneSpace);
		Joint.Transform = JointTransform;
		Joint.RestingAngles = JointTransform.Rotator();
		OTHER_LOG(Log, "Getting info for bone %s, %f, %f, %f, %f", *Joint.JointName, Joint.RestingAngles.Pitch,
		          Joint.RestingAngles.Yaw, Joint.RestingAngles.Roll);
		if (i > 0)
		{
...
void USpringBasedVegetationComponent::BeginPlay()
{
	...
	OTHER_LOG(Warning, "Params: BaseSpringStrength %f, Num joints: %d, CollisionForceParameter %f", BaseSpringStrength,
	          Skeleton.Joints.Num(), CollisionForceParameter);
	if (!SkeletalMesh)
	{
		UActorComponent* Component = GetOwner()->GetComponentByClass(USkeletalMeshComponent::StaticClass());
		SkeletalMesh = Cast<USkeletalMeshComponent>(Component);
...
void USpringBasedVegetationComponent::GenerateCollisionCapsules()
{
	for (FSkeletonJoint& Joint : Skeleton.Joints)
	{
		for (FSkeletonBone& Bone : Joint.Bones)
		{
			...
			Capsule->AttachToComponent(SkeletalMesh, FAttachmentTransformRules::KeepRelativeTransform,
			                           FName(*Joint.JointName));
			...
			else
			{
				...
				Capsule->OnComponentBeginOverlap.
				         AddDynamic(this, &USpringBasedVegetationComponent::OnBeginOverlapEvent);
...
void USpringBasedVegetationComponent::ComputeSpringStrengthForBranches()
{
	...
	for (FSkeletonJoint& Joint : Skeleton.Joints)
	{
		...
		OTHER_LOG(Log, "Joint: %s, location %s, Strength %f", *Joint.JointName, *JointLocation.ToString(),
		          Joint.SpringStrength);
	}
}
	...
	for (FSkeletonJoint& Joint : Skeleton.Joints)
	{
		...
		for (FSkeletonBone& Bone : Joint.Bones)
		{
			...
			LocalCylinderInertia <<
				...
				0.f, 1.f / 12.f * Bone.Mass * (3 * CylinderRadius * CylinderRadius + CylinderHeight * CylinderHeight),
				0.f,
				0.f, 0.f, 1.f / 12.f * Bone.Mass * (3 * CylinderRadius * CylinderRadius + CylinderHeight *
					CylinderHeight);
			...
			Eigen::Matrix3d JointSpaceInertia = LocalToJointMatrix * LocalCylinderInertia * (LocalToJointMatrix.
				transpose());
			Properties.InertiaTensor += Properties.JointToGlobalMatrix * JointSpaceInertia * Properties.
				JointToGlobalMatrix.transpose();
			Eigen::Vector3d BoneCenterWorldSpace = ToEigenVector(
				Joint.GlobalTransform.TransformPositionNoScale(Bone.CenterOfMass)) / 100.0;
			Properties.InertiaTensor += Properties.Mass * OuterProduct(Properties.CenterOfMass - BoneCenterWorldSpace,
			                                                           Properties.CenterOfMass - BoneCenterWorldSpace);
		}
		ACC_LOG(Log, "Local Joint: %s \n Inertia \n %s \n Force \n %s \n Torque \n %s \n COM: \n %s \n Mass %f",
		        *Joint.JointName, *EigenToFString(Properties.InertiaTensor), *EigenToFString(Properties.Force),
		        *EigenToFString(Properties.Torque), *EigenToFString(Properties.CenterOfMass), Properties.Mass);
	}
}
	...
	std::vector<FJointProperties>& JointPropertiesList)
{
	...
	for (int Id : Skeleton.EndToRootOrder)
	{
		...
		for (int ChildrenId : Joint.ChildrenIds)
		{
			FSkeletonJoint& ChildrenJoint = Skeleton.Joints[ChildrenId];
			FJointProperties& ChildrenProperties = JointPropertiesList[ChildrenId];
			Eigen::Vector3d ChildrenJointGlobalPosition = ToEigenVector(ChildrenJoint.GlobalTransform.GetLocation()) /
				100.f;
			Eigen::Vector3d ParentJointGlobalPosition = ToEigenVector(Joint.GlobalTransform.GetLocation()) / 100.f;
			JointProperties.Torque += ChildrenProperties.Torque + (ChildrenJointGlobalPosition -
				ParentJointGlobalPosition).cross(ChildrenProperties.Force);
		}

		// compute inertia tensor
		JointProperties.InertiaTensor = JointLocalProperties.InertiaTensor + JointLocalProperties.Mass * OuterProduct(
			JointProperties.CenterOfMass - JointLocalProperties.CenterOfMass,
			JointProperties.CenterOfMass - JointLocalProperties.CenterOfMass);
		for (int ChildrenId : Joint.ChildrenIds)
		{
			FSkeletonJoint& ChildrenJoint = Skeleton.Joints[ChildrenId];
			...
			JointProperties.InertiaTensor += ChildrenProperties.InertiaTensor + ChildrenProperties.Mass * OuterProduct(
				ChildrenToCenterOfMass, ChildrenToCenterOfMass);
		}
		ACC_LOG(Log, "Accumulated Joint: %s \n Inertia \n %s \n Force \n %s \n Torque \n %s \n COM: \n %s \n Mass %f",
		        *Joint.JointName, *EigenToFString(JointProperties.InertiaTensor),
		        *EigenToFString(JointProperties.Force), *EigenToFString(JointProperties.Torque),
		        *EigenToFString(JointProperties.CenterOfMass), JointProperties.Mass);
	}
}
	...
	std::vector<FJointProperties>& JointPropertiesList)
{
	...
	for (int i = 1; i < Skeleton.RootToEndOrder.Num(); i++)
	{
		...
		JointProperties.LinearAcceleration = ParentProperties.LinearAcceleration
			...
			+ ParentProperties.AngularVelocity.
			                   cross(JointProperties.AngularVelocity - ParentProperties.AngularVelocity);
		...
		FICT_LOG(Log, "Joint: %s \n Position \n %s \n Velocity \n %s \n Acceleration \n %s \n Fictitious forces: \n %s",
		         *Joint.JointName, *EigenToFString(CompositeCenterOfMass), *EigenToFString(CompositeLinearVelocity),
		         *EigenToFString(CompositeLinearAcceleration), *EigenToFString(JointProperties.FictitiousTorque));
	}
}
	...
	for (auto& ActorCapsules : OverlappingActors)
	{
		...
		for (UPrimitiveComponent* Capsule : CollidingCapsules)
		{
			...
			if (DebugEnableVisualization)
			{
				...
				DrawDebugLine(GetWorld(), ClosestPointOnCapsule, ClosestPointOnCollider, FColor::Green, false, 0.1f,
				              0.0f, 1.f);
			...
			const Eigen::Vector3d RepulsionForce = SpringTorque.cross(JointCapsuleVector) * JointCapsuleVector.
				squaredNorm();
			...
			const Eigen::Vector3d OverlappingForces = (ColliderPosition - PointOnCapsulePosition).normalized() *
				CollisionForceParameter * ForceFactor * Factor * Joint.CollisionForceProportionalFactor;
			Primitive->AddForceAtLocation(-ToUnrealVector(OverlappingForces) * 100.f, ClosestPointOnCollider);
			CollisionTorque += (PointOnCapsulePosition - JointGlobalPosition).cross(
				CollisionImpulse + OverlappingForces);
			...
			if (DebugEnableVisualization)
			{
				...
				DrawDebugLine(GetWorld(), ClosestPointOnCollider,
				              ClosestPointOnCollider + RepulsionForceUE.GetSafeNormal() * 20.f, FColor::Red, false,
				              0.1f, 0.0f, 1.f);
				FVector UEOverlapForces = ToUnrealVector(OverlappingForces) * 100.f;
				DrawDebugLine(GetWorld(), ClosestPointOnCapsule,
				              ClosestPointOnCapsule + UEOverlapForces.GetSafeNormal() * 20.f, FColor::Turquoise, false,
				              0.1f, 0.0f, 1.f);
	...
	float DeltaTime)
{
	...
	for (FSkeletonJoint& Joint : Skeleton.Joints)
	{
		...
		Eigen::Vector3d CenterToJoint = JointProperties.CenterOfMass - ToEigenVector(
			Joint.GlobalTransform.GetLocation()) / 100.f;
		...
		SOLVER_LOG(
			Log,
			"Bone %s: \n I: \n %s \n Tau: \n %s \n U: \n %s \n Lambda: \n %s \n X: \n %s \n GlobalToJoint \n %s \n CenterToJoint \n %s",
			*Joint.JointName, *StringI, *StringTorque, *StringU, *StringLambda, *EigenToFString(X),
			*StringGlobalToJoint, *StringCenterToJoint);
		...
		for (int i = 0; i < 3; i++)
		{
			...
			SOLVER_LOG(Log, "Bone %s, component %d, b %f, k %f, f %f, discriminant %f, theta0 %f", *Joint.JointName, i,
			           b, k, f, discriminant, theta0);
			if (k == 0)
			{
				NewTheta(i) = theta0;
			...
			else
			{
				...
				angularvelocity =
					c1 * std::exp(gamma * DeltaTime) * (gamma * std::cos(mu * DeltaTime) + mu *
						std::sin(mu * DeltaTime)) +
					c2 * std::exp(gamma * DeltaTime) * (gamma * std::sin(mu * DeltaTime) - mu *
						std::cos(mu * DeltaTime));
		...
		SOLVER_LOG(Log, "FinalNewTheta \n %s \n FinalNewThetaVelocity \n %s \n FinalNewThetaAccel \n %s",
		           *EigenToFString(FinalNewTheta), *EigenToFString(FinalNewThetaVelocity),
		           *EigenToFString(FinalNewThetaAccel));
	...
	const FHitResult& Hit)
{
	...
	COLLISION_LOG(LogCarla, Log, TEXT("Collision with bone %s, with impulse %s"), *Hit.MyBoneName.ToString(),
