...
void ASoilTypeManager::Tick(float DeltaTime)
{
	...
	{
		ALargeMapManager* LargeMapManager = (ALargeMapManager*)UGameplayStatics::GetActorOfClass(
			GetWorld(), ALargeMapManager::StaticClass());
		...
		if (Car != nullptr)
		{
			...
			UE_LOG(LogCarla, Log,
			       ...
			       TileIndex, TileVector.X, TileVector.Y, TileVector.Z, CarPos.X, CarPos.Y, CarPos.Z, GlobalCarPos.X,
			       GlobalCarPos.Y, GlobalCarPos.Z,
			       *TypeStr);
		}
	}
#endif
