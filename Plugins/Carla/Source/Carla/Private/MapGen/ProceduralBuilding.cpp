...
#include "MapGen/ProceduralBuilding.h"
	...
	const UStaticMesh* SM)
{
	...
	HISMComp = NewObject<UHierarchicalInstancedStaticMeshComponent>(this,
	                                                                FName(*FString::Printf(
		                                                                TEXT("HISMComp_%d"), HISMComps.Num())));
	...
	bool CornerVisbility)
{
	 ...
	 * Corner part
	 */
	bool AreCornerMeshesAvailable = (CornerMeshes && (CornerMeshes->Num() > 0)) || (CornerBPs && (CornerBPs->Num() >
		0));
