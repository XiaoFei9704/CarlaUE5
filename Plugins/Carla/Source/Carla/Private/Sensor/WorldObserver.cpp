...
#include "Sensor/WorldObserver.h"
#include "Carla.h"
		...
		if (TrafficLight != nullptr)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						UE_LOG(LogCarla, Warning, TEXT("The max size of a signal id is 32. %s (%d)"), *fstring_sign_id,
						       sign_id.length());
...
static carla::geom::Vector3D FWorldObserver_GetAngularVelocity(const AActor& Actor)
{
	...
	const FVector AngularVelocity =
		RootComponent != nullptr ? RootComponent->GetPhysicsAngularVelocityInDegrees() : FVector{0.0f, 0.0f, 0.0f};
