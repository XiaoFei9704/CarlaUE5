...
#include "Commandlet/PrepareAssetsForCooking.h"
		...
		if (MeshAsset && ValidateStaticMesh(MeshAsset))
		{
			...
			if (i == -1 || (i != -1 && (AssetName.EndsWith(TileName) || AssetName.Contains(TileName + "_"))))
			{
				...
				if (bUseCarlaMaterials)
				{
					...
					{
						for (int32 i = 0; i < MeshActor->GetStaticMeshComponent()->GetStaticMesh()->StaticMaterials.
						                                 Num(); ++i)
						{
							if (MeshActor->GetStaticMeshComponent()->GetStaticMesh()->StaticMaterials[i].
							    ImportedMaterialSlotName.ToString().Contains("Yellow"))
							{
	...
	const TArray<FMapData>& MapsPaths)
{
	...
	for (const auto& Map : MapsPaths)
	{
		...
		else
		{
			...
			FString TxtFile;
			FString TilesInfoPath = FPaths::ProjectContentDir() + PackageName + TEXT("/Maps/") + Map.Name +
				"/TilesInfo.txt";
			...
			do
			{
				...
				{
					// Spawn
					TArray<AStaticMeshActor*> SpawnedActors = SpawnMeshesToWorld(
						DataPath, Map.bUseCarlaMapMaterials, i, j);
