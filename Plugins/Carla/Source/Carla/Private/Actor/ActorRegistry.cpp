	...
	crp::ActorState InState)
{
	...
	const auto Type = FActorRegistry_GetActorType(&Actor);
...
void FActorRegistry::WakeActorUp(FCarlaActor::IdType Id, UCarlaEpisode* CarlaEpisode)
{
	...
	if (Actor)
	{
		...
		if (CarlaActor->GetParent())
		{
			...
			{
				UE_LOG(LogCarla, Error, TEXT("Failed to attach actor %d to %d during wake up"), Id,
				       CarlaActor->GetParent());
			}
		}
	}
	for (const FCarlaActor::IdType& ChildId : CarlaActor->GetChildren())
...
{
	for (const auto& Item : ActorDatabase)
	{
		// check for a sensor
