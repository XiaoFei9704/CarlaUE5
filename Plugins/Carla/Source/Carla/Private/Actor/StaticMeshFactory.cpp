...
#include "Actor/StaticMeshFactory.h"
	...
	const FActorDescription& ActorDescription)
{
	...
	const float Scale = ABFL::RetrieveActorAttributeToFloat("scale", ActorDescription.Variations, 1.0f);
	...
	if (StaticMeshComponent)
	{
		...
		{
			const FString MeshPath = ABFL::ActorAttributeToString(
			...
			{
				const float Mass = ABFL::RetrieveActorAttributeToFloat("mass", ActorDescription.Variations, 0.0f);
				if (Mass > 0)
				{
					StaticMeshComponent->SetMobility(EComponentMobility::Movable);
